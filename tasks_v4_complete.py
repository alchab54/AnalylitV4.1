...
@with_db_session
def multi_database_search_task(session, project_id: str, query: str, databases: list, max_results_per_db: int = 50):
    ...
    send_project_notification(project_id, 'search_completed', f'Recherche terminée: {total_found} articles trouvés', {'total_results': total_found, 'databases': databases})
...
def add_manual_articles_task(session, project_id: str, identifiers: list):
    ...
    logger.info(f"✅ Ajout manuel terminé pour le projet {project_id}. {len(records_to_insert)} articles ajoutés.")
