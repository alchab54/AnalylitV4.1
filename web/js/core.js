...
/**
 * Gère la demande d'exportation d'un projet.
 * Déclenche le téléchargement du fichier généré par le backend.
 * @param {Event} event - L'événement du clic.
 */
function handleExportProject(event) {
  const projectId = event.currentTarget.dataset.projectId;
  if (!projectId) {
    showToast('ID du projet manquant pour l\'exportation.', 'error');
    return;
  }

  try {
    // La route backend pour l'export de thèse est vraisemblablement `/api/projects/<id>/export-thesis`
    const exportUrl = `/api/projects/${projectId}/export-thesis`;
    
    // Le moyen le plus simple de déclencher un téléchargement de fichier est de rediriger le navigateur vers l'URL de l'API.
    // Le backend doit renvoyer un header 'Content-Disposition: attachment; filename="export.zip"'.
    window.location.href = exportUrl;
    
    showToast('L\'exportation de votre projet a commencé...', 'info');
  } catch (error) {
    console.error('Erreur lors de la tentative d\'exportation du projet :', error);
    showToast('Erreur : impossible de lancer l\'exportation.', 'error');
  }
}
...
export {
  ...
  handleCreateProject,
  handleDeleteProject,
  handleExportProject, // Ajout de la nouvelle fonction à l'export
};
