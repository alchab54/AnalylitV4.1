# utils/fetchers.py - Module de récupération de données externes
import logging
import requests
import time
import json
...
class DatabaseManager:
    ...
    def search_pubmed(self, query: str, max_results: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Parse XML simple (à adapter selon vos besoins)
            results = []
            for pmid in pmids:
                results.append({
                    "id": pmid,
    ...
    def search_arxiv(self, query: str, max_results: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Parse XML basique (à améliorer)
            results = []
            # Simulation simple - à remplacer par un vrai parser XML
            for i in range(min(5, max_results)):  # Simulation
                results.append({
                    "id": f"arxiv_{i}",
    ...
    def search_crossref(self, query: str, max_results: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in data.get("message", {}).get("items", []):
                title = item.get("title", ["Titre non disponible"])[0]
                doi = item.get("DOI", "")
                ...
                results.append({
                    ...
                    "publication_date": str(item.get("published-print", {}).get("date-parts", [[2024]])[0][0]),
    ...
    def search_ieee(self, query: str, max_results: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            # Simulation IEEE (nécessite une clé API)
            results = []
            for i in range(min(3, max_results)):  # Simulation
                results.append({
                    "id": f"ieee_{i}",
...
def fetch_unpaywall_pdf_url(doi: str) -> Optional[str]:
    ...
    try:
        ...
        # Cherche l'URL du PDF en accès libre
...
def fetch_article_details(article_id: str) -> Dict[str, Any]:
    ...
    try:
        # Détermine le type d'ID et la source
...
def _fetch_pubmed_details(pmid: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Parse XML basique (à améliorer avec xml.etree)
        return {
            "id": pmid,
            "title": f"Article PubMed {pmid}",
            "abstract": "Résumé à parser depuis XML",
            "authors": "Auteurs à parser",
            "publication_date": "2024",
            "journal": "Journal à parser",
            "doi": "",
...
def _fetch_crossref_details(doi: str) -> Dict[str, Any]:
    """Récupère les détails d'un article via CrossRef."""
    try:
...
def _fetch_arxiv_details(arxiv_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "id": arxiv_id,
            "title": f"Article arXiv {arxiv_id}",
            "abstract": "Résumé arXiv à parser",
            "authors": "Auteurs arXiv",
            "publication_date": "2024",
            "journal": "arXiv preprint",
            "doi": "",
