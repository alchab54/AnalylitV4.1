...
# Get config for Redis URL
config = get_config()

# Initialize a SocketIO object specifically for workers.
# This connects them to the same Redis message queue as the web server,
# allowing them to emit messages without the full Flask app context.
...
def send_project_notification(project_id: str, notification_type: str, message: str, data: Optional[Dict[str, Any]] = None):
    ...
    try:
        ...
        # Use the worker-specific SocketIO instance to emit
...
def send_global_notification(notification_type: str, message: str, data: Optional[Dict[str, Any]] = None):
    ...
    try:
        ...
        # Use the worker-specific SocketIO instance to emit globally
