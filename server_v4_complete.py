...
def list_project_extractions(project_id):
    ...
    try:
        ...
        logger.error(f"Erreur /projects/<project_id>/extractions: {e}", exc_info=True)
        return jsonify([]), 404
    finally:
        session.close()
...
# --- GESTION DES FILES D'ATTENTE (UNIFIÉE) ---
...
def queue_status():
    try:
        ...
        for name, q in queues.items():
            data[name] = {
                'enqueued': q.count,
                'started': len(StartedJobRegistry(queue=q).get_job_ids()),
                'finished': len(FinishedJobRegistry(queue=q).get_job_ids()),
                'failed': len(FailedJobRegistry(queue=q).get_job_ids()),
                'deferred': len(DeferredJobRegistry(queue=q).get_job_ids()),
            }

        return jsonify(data), 200
    except Exception as e:
        logger.error(f"Erreur queue_status: {e}", exc_info=True)
...
def queues_clear():
    try:
        ...
        if qname not in mapping:
            return jsonify({'error': f"unknown queue '{qname}'"}), 400

        q = mapping[qname]
        q.empty()

        for reg_cls in (FailedJobRegistry, StartedJobRegistry, FinishedJobRegistry, DeferredJobRegistry):
            reg = reg_cls(queue=q)
            for jid in reg.get_job_ids():
                try:
                    reg.remove(jid, delete_job=True)
                except Exception as e:
                    logger.warning(f"Remove {jid} from {reg_cls.__name__} failed: {e}")

        return jsonify({'message': f"Queue '{qname}' cleared."}), 200
    except Exception as e:
        logger.error(f"Erreur queues_clear: {e}", exc_info=True)
        return jsonify({'error': 'internal', 'details': str(e)}), 500
