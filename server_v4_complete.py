...
def fetch_online_pdfs(project_id):
    ...
    # Fix: Boucle côté serveur pour chaque article individuellement
...
def import_validations(project_id):
    ...
    try:
        ...
        return jsonify({"message": f"{updated} validations importées pour l'évaluateur."})
...
def run_atn_score(project_id):
    job = analysis_queue.enqueue(run_atn_score_task, project_id=project_id, job_timeout='30m')
    return jsonify({'message': 'Calcul du score ATN lancé.', 'job_id': job.id}), 202
...
def handle_join_room(data):
    room = data.get('room')
    if room:
        logger.info(f"Client rejoint la room: {room}")
