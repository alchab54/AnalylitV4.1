# AnalyLit V4.0 - Serveur Flask COMPLET avec gestion complète des profils
...
def init_db():
    """Initialise la base de données pour la V4 avec toutes les tables nécessaires."""
    with sqlite3.connect(DATABASE_FILE) as conn:
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS projects (
                id TEXT PRIMARY KEY, name TEXT NOT NULL, description TEXT, status TEXT DEFAULT 'pending',
                profile_used TEXT, job_id TEXT, created_at TEXT, updated_at TEXT, synthesis_result TEXT,
                discussion_draft TEXT, knowledge_graph TEXT, prisma_flow_path TEXT, analysis_mode TEXT DEFAULT 'screening',
                analysis_result TEXT, analysis_plot_path TEXT, pmids_count INTEGER DEFAULT 0,
                processed_count INTEGER DEFAULT 0, total_processing_time REAL DEFAULT 0
            )""")
        
        c.execute("""
            CREATE TABLE IF NOT EXISTS extractions (
                id TEXT PRIMARY KEY, project_id TEXT, pmid TEXT, title TEXT, validation_score REAL, created_at TEXT,
                extracted_data TEXT, relevance_score REAL DEFAULT 0, relevance_justification TEXT,
                user_validation_status TEXT, FOREIGN KEY (project_id) REFERENCES projects(id)
            )""")
        
        c.execute("""
            CREATE TABLE IF NOT EXISTS processing_log (
                id INTEGER PRIMARY KEY AUTOINCREMENT, project_id TEXT, pmid TEXT, status TEXT,
                details TEXT, timestamp TEXT, FOREIGN KEY (project_id) REFERENCES projects(id)
            )""")
        
        c.execute("""
            CREATE TABLE IF NOT EXISTS analysis_profiles (
                id TEXT PRIMARY KEY, name TEXT NOT NULL UNIQUE, is_custom BOOLEAN DEFAULT 1,
                preprocess_model TEXT NOT NULL, extract_model TEXT NOT NULL, synthesis_model TEXT NOT NULL
            )""")
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS extraction_grids (
                ...
                fields TEXT NOT NULL, created_at TEXT, FOREIGN KEY (project_id) REFERENCES projects(id)
            )""")
        
        c.execute("""
            CREATE TABLE IF NOT EXISTS chat_messages (
                id TEXT PRIMARY KEY, project_id TEXT NOT NULL, role TEXT NOT NULL,
                content TEXT NOT NULL, sources TEXT, timestamp TEXT,
        ...
        if c.execute("SELECT COUNT(*) FROM prompts").fetchone()[0] == 0:
            default_prompts = [
                ...
                """En tant qu'assistant de recherche spécialisé en biomédecine, analysez cet article et déterminez sa pertinence pour une revue systématique.
...
def health_check():
    ...
    return jsonify({
        "status": "ok", "version": config.ANALYLIT_VERSION,
...
# Endpoint pour sauvegarder les paramètres Zotero
@api_bp.route('/settings/zotero', methods=['POST'])
def save_zotero_settings():
    data = request.get_json()
    with open(PROJECTS_DIR / 'zotero_config.json', 'w') as f:
        json.dump({'user_id': data.get('userId'), 'api_key': data.get('apiKey')}, f)
    return jsonify({'message': 'Paramètres Zotero sauvegardés.'})

# Endpoint pour lancer l'import depuis Zotero
...
def import_from_zotero(project_id):
    ...
    # Récupérer les PMIDs depuis la base de données pour le projet actuel
    with sqlite3.connect(DATABASE_FILE) as conn:
        pmids_cursor = conn.execute("SELECT pmid FROM extractions WHERE project_id = ?", (project_id,)).fetchall()
        pmids = [row[0] for row in pmids_cursor]
    ...
    background_queue.enqueue(
        ...
        pmids=pmids,
        zotero_user_id=config.get('user_id'),
        zotero_api_key=config.get('api_key'),
        job_timeout='1h'
    )
    
    return jsonify({'message': f'Import depuis Zotero lancé pour {len(pmids)} articles.'}), 202
...
def upload_pdfs_bulk(project_id):
    ...
    for file in files:
        if file and file.filename:
            # Extrait le PMID du nom de fichier (ex: 12345678.pdf)
            pmid = Path(file.filename).stem
    ...
    return jsonify({'successful': successful, 'failed': failed, 'skipped': []})
    
# Endpoint pour lancer la recherche de PDF en ligne
...
def fetch_online_pdfs(project_id):
    ...
    return jsonify({'message': f'La recherche de PDF en ligne a été lancée pour {len(pmids)} articles.'}), 202
...
# Endpoint pour lancer l'indexation
...
def run_indexing(project_id):
    ...
    return jsonify({'message': 'L\'indexation du corpus a été lancée.', 'job_id': job.id}), 202

# Endpoints pour le chat
@api_bp.route('/projects/<project_id>/chat', methods=['POST'])
def handle_chat_message(project_id):
    """Reçoit une question de l'utilisateur, la traite et renvoie la réponse."""
    data = request.get_json()
...
def get_chat_history(project_id):
    """Récupère l'historique de la conversation pour un projet."""
    with sqlite3.connect(DATABASE_FILE) as conn:
        conn.row_factory = sqlite3.Row
        messages = conn.execute(
            "SELECT role, content, sources, timestamp FROM chat_messages WHERE project_id = ? ORDER BY timestamp ASC",
            (project_id,)
        ).fetchall()
...
def pull_ollama_model():
    ...
    return jsonify({'message': f'Le téléchargement du modèle "{model_name}" a été lancé.', 'job_id': job.id}), 202
...
# Routes pour les profils d'analyse
...
def create_analysis_profile():
    ...
    if not all(field in data for field in required_fields):
        return jsonify({'error': 'Tous les champs sont requis: name, preprocess_model, extract_model, synthesis_model'}), 400
...
def create_project():
    ...
    with sqlite3.connect(DATABASE_FILE) as conn:
        conn.execute(
            "INSERT INTO projects (id, name, description, created_at, updated_at, analysis_mode) VALUES (?, ?, ?, ?, ?, ?)",
...
# Routes pour les grilles d'extraction
@api_bp.route('/projects/<project_id>/grids', methods=['POST'])
def create_extraction_grid(project_id):
    """Crée une nouvelle grille d'extraction pour un projet."""
    data = request.get_json()
    name = data.get('name')
    ...
    try:
        with sqlite3.connect(DATABASE_FILE) as conn:
            conn.execute(
                "INSERT INTO extraction_grids (id, project_id, name, fields, created_at) VALUES (?, ?, ?, ?, ?)",
    ...
    try:
        with sqlite3.connect(DATABASE_FILE) as conn:
            ...
            grids_cursor = conn.execute(
                "SELECT id, name, fields, created_at FROM extraction_grids WHERE project_id = ? ORDER BY created_at DESC",
                (project_id,)
            ).fetchall()
...
def update_extraction_grid(project_id, grid_id):
    """Met à jour une grille d'extraction existante."""
    data = request.get_json()
    name = data.get('name')
    fields = data.get('fields')
    ...
    try:
        with sqlite3.connect(DATABASE_FILE) as conn:
            cursor = conn.execute(
                "UPDATE extraction_grids SET name = ?, fields = ? WHERE id = ? AND project_id = ?",
...
def delete_extraction_grid(project_id, grid_id):
    ...
    try:
        with sqlite3.connect(DATABASE_FILE) as conn:
            cursor = conn.execute(
                "DELETE FROM extraction_grids WHERE id = ? AND project_id = ?",
                (grid_id, project_id)
            )
...
def run_project_pipeline(project_id):
    ...
    if not pmids:
        return jsonify({'error': 'PMIDs requis'}), 400
    ...
    with sqlite3.connect(DATABASE_FILE) as conn:
        ...
        conn.execute(
            """UPDATE projects SET
               ...
               WHERE id = ?""",
            (profile_id, datetime.now().isoformat(), len(pmids), project_id)
        )
        conn.commit()
    
    # Lancer le traitement pour chaque PMID (sauf en mode validation)
    ...
    return jsonify({
        "status": "processing",
        "message": f"{len(pmids)} articles sont en cours de traitement."
    }), 202
...
def run_synthesis_endpoint(project_id):
    ...
    return jsonify({"status": "synthesizing", "message": "La synthèse des résultats a été lancée."}), 202

# Routes pour les extractions
...
# Routes pour les logs et résultats
...
# Routes pour les images et fichiers
...
# Upload de PDF
@api_bp.route('/projects/<project_id>/<pmid>/upload-pdf', methods=['POST'])
def upload_pdf(project_id, pmid):
    ...
    return jsonify({'message': f'PDF pour {pmid} importé avec succès.'}), 200
    ...
    if file and file.filename.endswith('.json'):
        try:
            ...
            with sqlite3.connect(DATABASE_FILE) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO extraction_grids (id, project_id, name, fields) VALUES (?, ?, ?, ?)",
                    (str(uuid.uuid4()), project_id, grid_name, json.dumps(grid_fields))
                )
                conn.commit()
...
if __name__ == '__main__':
    init_db()
    logger.info("🚀 Démarrage du serveur de production gevent sur le port 5001...")
