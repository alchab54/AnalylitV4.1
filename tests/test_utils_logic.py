...
def test_generate_discussion_draft_logic()
    ...
    assert Résultat C not in prompt_arg
    assert pmid3 not in prompt_arg
...
def test_fetchers_arxiv_and_crossref(mock_get):
            ...
            items [
                {
                    ...
                    abstract pRésumé CrossRef.p,
                    DOI 10.1234cr.1,
                    URL httpdoi.org10.1234cr.1,
    ...
    assert results_crossref[0]['id'] == 10.1234cr.1
...
def test_import_from_zotero_json_task_logic(mock_notify, mock_session, db_session, setup_project):
    ...
    existing_article = SearchResult(
        ...
        article_id=10.1000existing # L'ID unique
    )
